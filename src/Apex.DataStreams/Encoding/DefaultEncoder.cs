...
namespace Apex.DataStreams.Encoding {
    ...
    internal sealed class DefaultEncoder : IEncoder {
        ...
        public DefaultEncoder(ISerializer serializer) {
        ...
        public MessageEnvelope Encode(DataStreamTopicDefinition topicDefinition, object message) {
            ...
            using (var ms = new MemoryStream(messageDefinition.MessageBufferSize)) {
                ...
                /// Disposing the BsonDataWriter here makes sure all the message body is flushed into the MemoryStream, so that when we do call 
                /// ms.ToArray down below, we get all the data. (If we don't dispose the writer first, ms.ToArray() could return part but not all of the bytes.)
                using (var writer = new BsonDataWriter(ms)) {
                    ...
                    /// And finally we return the byte[] using an ArraySegment, because the preferred Socket.SendAsync() method accepts an ArraySegment<byte> parameter.
                    var segment = new ArraySegment<byte>(ms.ToArray());
                    return new MessageEnvelope(topicDefinition.TopicCode, messageDefinition.MessageCode, messageType, segment, message);
                }
            }
        }
        ...
        public async Task<MessageEnvelope> ReadMessageFromSocketAsync(DataStreamDefinition dataStreamDefinition, Socket socket) {
            ...
            var header = await ReadBytesAsync(4).ConfigureAwait(false);
            ...
            var messageBytes = await ReadBytesAsync(messageBodyLength).ConfigureAwait(false);
            using (var ms = new MemoryStream(messageBytes))
            using (var reader = new BsonDataReader(ms)) {
                try {
                    var message = Serializer.Deserialize(reader, messageDefinition.MessageType);
                    return new MessageEnvelope(header[0], header[1], messageDefinition.MessageType, new ArraySegment<byte>(), message);
                } catch (Exception x) {
            ...
            async Task<byte[]> ReadBytesAsync(int totalBytesRequired) {
                var bytes = new byte[totalBytesRequired];
                var totalBytesRead = 0;
                while (totalBytesRead < totalBytesRequired) {
                    var segment = new ArraySegment<byte>(bytes, totalBytesRead, totalBytesRequired - totalBytesRead);
                    var bytesActuallyRead = await socket.ReceiveAsync(segment, SocketFlags.None).ConfigureAwait(false);
                    if (bytesActuallyRead == 0) throw new Exception($"Socket was disconnected while trying to {totalBytesRequired} bytes.");
